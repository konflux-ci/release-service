...
var _ = Describe("PipelineRun builder", func() {
	...
	When("WithPipelineRef method is called", func() {
		...
		It("works correctly when revision is already a SHA", func() {
			...
			Expect(builder.pipelineRun.Spec.Params[2].Name).To(Equal("pipelineGitRevision"))
			Expect(builder.pipelineRun.Spec.Params[2].Value.StringVal).To(Equal(originalSHA))
