...
var _ = ginkgo.Describe("Managed pipeline OOM is not retried when disabled by a template variable tag in the Release CR", releasecommon.LabelManagedPipelineDisabledByReleaseTagNoRetry, ginkgo.Ordered, func() {
	defer ginkgo.GinkgoRecover()

	var fw *framework.Framework
	ginkgo.AfterEach(framework.ReportFailure(&fw))
	var err error
	var devNamespace = "retry-release-tag-disabled"
	var managedNamespace = "retry-release-tag-disabled-managed"
	...
	var ecPolicyName = "retry-release-tag-disabled-policy-" + utils.GenerateRandomString(4)
	var releaseName = "retry-release-tag-disabled"
	...
	ginkgo.BeforeAll(func() {
		...
		// The disable tags use regex patterns to match unresolved template variable tags in the Release CR data.
		...
		// The RPA has no tags in its data — the disable tag comes solely from the Release CR below.
		...
		snapshot, err := fw.AsKubeAdmin.IntegrationController.CreateSnapshotWithImageSource(constants.ComponentName, constants.ApplicationNameDefault, devNamespace, sampleImage, constants.GitSourceComponentUrl, gitSourceRevision, "", "", "", "")
		gomega.Expect(err).ShouldNot(gomega.HaveOccurred(), "failed to create Snapshot: %v", err)
		snapshotName = snapshot.Name

		releaseData, err := json.Marshal(map[string]interface{}{
		...
		gomega.Expect(err).NotTo(gomega.HaveOccurred(), "failed to marshal Release data: %v", err)

		_, err = fw.AsKubeAdmin.ReleaseController.CreateRelease(releaseName, devNamespace, snapshotName, constants.SourceReleasePlanName, &runtime.RawExtension{Raw: releaseData})
		gomega.Expect(err).NotTo(gomega.HaveOccurred(), "failed to create Release %s: %v", releaseName, err)
	})
	...
	var _ = ginkgo.Describe("Post-release verification", func() {
		...
		ginkgo.It("verifies that the Release CR exists in the dev namespace", func() {
		...
		ginkgo.It("verifies the Release failed with a single OOM attempt and no retries due to tags in the Release CR", func() {
			gomega.Expect(framework.GetManagedPipelineRetryCount(releaseCR)).To(gomega.Equal(0), "expected no retries when disabled by template variable tags in the Release CR data")
