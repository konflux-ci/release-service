...
func (a *Adapter) EnsureReleaseIsCompleted() (reconciler.OperationResult, error) {
	a.logger.Info("EnsureReleaseIsCompleted", "Release Status", a.release.Status)
	a.logger.Info("EnsureReleaseIsCompleted", "HasReleaseFinished", a.release.HasReleaseFinished())
	// Do nothing if the release status has been already added
	if a.release.HasReleaseFinished() {
		a.logger.Info("EnsureReleaseIsCompleted", "HasReleaseFinished", "Within loop")
		return reconciler.ContinueProcessing()
	}

	// The processing has to complete for a Release to be completed
	if !a.release.HasProcessingFinished() {
		a.logger.Info("EnsureReleaseIsCompleted", "HasProcessingFinished", "Within loop")
		return reconciler.ContinueProcessing()
	}
	...
	a.logger.Info("EnsureReleaseIsCompleted", "MarkReleased", "Fuck!")
	patch := client.MergeFrom(a.release.DeepCopy())
...
func (a *Adapter) EnsureReleaseIsDeployed() (reconciler.OperationResult, error) {
	if !a.release.IsProcessed() || a.release.HasDeploymentFinished() {
		return reconciler.ContinueProcessing()
	}
