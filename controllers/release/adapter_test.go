...
var _ = Describe("Release adapter", Ordered, func() {
	...
	When("cleanupManagedPipelineResources is called", func() {
		...
		It("should cleanup PipelineRuns for all attempts", func() {
			...
			cleaned, err = adapter.loader.GetReleasePipelineRunAttempt(adapter.ctx, adapter.client, adapter.release, 1)
			Expect(err).NotTo(HaveOccurred())
			Expect(cleaned).NotTo(BeNil())
			Expect(cleaned.Finalizers).To(HaveLen(0))
			Expect(k8sClient.Delete(ctx, secondPipelineRun)).To(Succeed())
		})
		...
		It("should cleanup the shared RoleBinding from the first attempt", func() {
			...
			cleaned, err := adapter.loader.GetRoleBindingFromPipelineAttempt(
				adapter.ctx, adapter.client, &adapter.release.Status.ManagedPipelineAttempts[0])
			Expect(errors.IsNotFound(err)).To(BeTrue())
			Expect(cleaned).To(BeNil())
	...
	When("retryManagedPipeline is called", func() {
		...
		It("should create a new PipelineRun and register the attempt", func() {
			adapter.release.Status.ManagedPipelineAttempts = []v1alpha1.PipelineAttempt{{
				PipelineRun:   "default/pr",
				Status:        v1alpha1.AttemptFailedReason,
				FailureReason: v1alpha1.AttemptFailureOOMKillReason,
			...
			Expect(adapter.release.Status.ManagedPipelineAttempts[1].PipelineRun).NotTo(BeEmpty())
		})

		It("should return an error when processing resources cannot be loaded", func() {
			adapter.ctx = toolkit.GetMockedContext(ctx, []toolkit.MockData{
				{
					ContextKey: loader.ProcessingResourcesContextKey,
