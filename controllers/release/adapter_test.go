...
var _ = Describe("Release adapter", Ordered, func() {
	...
	When("EnsureManagedPipelineProcessingIsCompleted is called", func() {
		...
		It("should finalize the failure when retries are disabled by release data tags", func() {
			...
			retryRpa.Status.RetryInfo = &v1alpha1.RetryInfo{
				Enabled:       true,
				MaxRetries:    &maxRetries,
				DisablingTags: []string{"production"},
			}
	...
	When("canRetryManagedPipeline is called", func() {
		var adapter *adapter
		var retryRpa *v1alpha1.ReleasePlanAdmission
		...
		It("should return true when the attempt is retriable, retries are enabled and retries are left", func() {
			adapter.release.Status.ManagedPipelineAttempts = []v1alpha1.PipelineAttempt{{PipelineRun: "default/pr"}}
			adapter.release.MarkCurrentManagedPipelineAttemptProcessing()
			adapter.release.MarkCurrentManagedPipelineAttemptFailed("oom", v1alpha1.AttemptFailureOOMKillReason, "", "", 0)

			Expect(adapter.canRetryManagedPipeline(retryRpa)).To(BeTrue())
		})

		It("should return false when the attempt is not retriable", func() {
			adapter.release.Status.ManagedPipelineAttempts = []v1alpha1.PipelineAttempt{{PipelineRun: "default/pr"}}
			adapter.release.MarkCurrentManagedPipelineAttemptProcessing()
			adapter.release.MarkCurrentManagedPipelineAttemptFailed("err", v1alpha1.AttemptFailureErrorReason, "", "", 0)

			Expect(adapter.canRetryManagedPipeline(retryRpa)).To(BeFalse())
		})

		It("should return false when retries are not enabled", func() {
			retryRpa.Status.RetryInfo.Enabled = false
			adapter.release.Status.ManagedPipelineAttempts = []v1alpha1.PipelineAttempt{{PipelineRun: "default/pr"}}
			adapter.release.MarkCurrentManagedPipelineAttemptProcessing()
			adapter.release.MarkCurrentManagedPipelineAttemptFailed("oom", v1alpha1.AttemptFailureOOMKillReason, "", "", 0)
		...
		It("should return false when there are no retries left", func() {
			adapter.release.Status.ManagedPipelineAttempts = []v1alpha1.PipelineAttempt{
				{PipelineRun: "default/pr-1"},
				{PipelineRun: "default/pr-2"},
			}
