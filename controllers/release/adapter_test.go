...
var _ = Describe("Release adapter", Ordered, func() {
	...
	When("emitValidationFailureWaitSpan is called", func() {
		...
		It("preserves in-memory status mutations on a.release across the annotation patch", func() {
			prev := otel.GetTracerProvider()
			tp := sdktrace.NewTracerProvider()
			otel.SetTracerProvider(tp)
			DeferCleanup(func() {
			...
			// In-memory override; not committed to server.
			adapter.release.MarkValidated()
			Expect(apimeta.FindStatusCondition(adapter.release.Status.Conditions, "Validated").Status).To(Equal(metav1.ConditionTrue))

			adapter.emitValidationFailureWaitSpan()

			cond := apimeta.FindStatusCondition(adapter.release.Status.Conditions, "Validated")
